...
import 'package:sleepmohapp/UI/B1_Home/Hotel/Hotel_Detail_Concept_2/tourismedetails.dart';
...
var compteU;
var listTourisme = initlisTourisme.map((model) => Tourisme.fromJson(model)).toList();
var listrestau = initlisTourisme.map((model) => Tourisme.fromJson(model)).toList();
var listPark = initlisTourisme.map((model) => Tourisme.fromJson(model)).toList();
var listloisir = initlisTourisme.map((model) => Tourisme.fromJson(model)).toList();
var listsitetouris = initlisTourisme.map((model) => Tourisme.fromJson(model)).toList();
...
void initSaveData () async {
   ...
   setState(() {
   //  log("message");
   var valutourisme = sharedPrefs.get('listTourisme');
     //listHotels = jsonDecode(sharedPrefs.get('listHotel1'));
//print(valuHotel);
  //  print("listhPref");
     if (valutourisme != null){
       Iterable list0 = jsonDecode(valutourisme);
        listTourisme = list0.map((model) => Tourisme.fromJson(model)).toList();
     } 

      listloisir = listTourisme.where((element) => element.type == "Loisir").toList();
      listsitetouris = listTourisme.where((element) => element.type == "Site touristique").toList();
      listrestau = listTourisme.where((element) => element.type == "Restaurant").toList();
      listPark = listTourisme.where((element) => element.type == "Park national").toList();
  ...
  Widget build(BuildContext context) {
    ...
    var _slideImage = Container(
      ...
      child: new Carousel(
        ...
        images: [
          CachedNetworkImageProvider("https://sleepmoh.com/flutterimg/camer0.jpg"),
          CachedNetworkImageProvider("https://sleepmoh.com/flutterimg/camer1.jpg"),
          CachedNetworkImageProvider("https://sleepmoh.com/flutterimg/camer22.jpg"),
          CachedNetworkImageProvider("https://sleepmoh.com/flutterimg/camer33.png"),
        ],
      ),
    );
      ...
      children: <Widget>[
        ...
        Padding(
          ...
          child: Container(
              ...
              child: Text(
                AppLocalizations.of(context).tourisdest,
                style: TextStyle(
                    fontFamily: "Sofia",
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: ListView(
            ...
            children:listrestau.map<Widget>((data) => _card(data.imgh, data.nomtr, data.localisation, "0.1", data, context)).toList(),
          ),
        ),
  SizedBox(
          height: 25.0,
        ...
        Container(
          ...
          child: ListView(
            scrollDirection: Axis.horizontal,
            children:listsitetouris.map<Widget>((data) => _card(data.imgh, data.nomtr, data.localisation, "0.1", data, context)).toList(),
          ),
        ),
        ...
        Container(
          ...
          child: ListView(
            scrollDirection: Axis.horizontal,
            children:listloisir.map<Widget>((data) => _card(data.imgh, data.nomtr, data.localisation, "0.1", data, context)).toList(),
          ),
        ),
        SizedBox(
          height: 25.0,
        ...
        Container(
          ...
          child: ListView(
            scrollDirection: Axis.horizontal,
            children:listPark.map<Widget>((data) => _card(data.imgh, data.nomtr, data.localisation, "0.1", data, context)).toList(),
          ),
        ),
      ],
    );
...
Widget _card(String image, title, location, ratting, Tourisme data, BuildContext context) {
  return Padding(
    padding: const EdgeInsets.all(8.0),
    child: InkWell(
        onTap: () {
          Navigator.of(context).push(PageRouteBuilder(
              pageBuilder: (_, __, ___) => new tourismedetails(
                    title: title,
                    id: data.id,
                    image: data.imgh,
