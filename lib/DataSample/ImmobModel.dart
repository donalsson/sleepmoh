...
class ImmobMod {
  ...
  static String encodeHotels(List<ImmobMod> hotelhts) => json.encode(
        hotelhts
            .map<Map<String, dynamic>>((hotelht) => toMap(hotelht))
            .toList(),
      );

    static List<ImmobMod> decodeHotels(String hotelhts) =>
      (json.decode(hotelhts) as List<dynamic>)
          .map<ImmobMod>((item) => ImmobMod.fromJson(item))
          .toList();
